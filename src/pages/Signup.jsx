...
function Signup() {
    ...
    const handleSubmit = async (e) => {
        ...
        const { data: userData, error: userError } = await supabase
            .from("Users")
            .insert([{ famUser: username, pass_hash: password }])
            .select("famID")
            .single();
        ...
        const currFamID = userData.famID;

        // If your Members table column is "Is_adult" (capital I), change the key here.
        const membersRows = members.map((m) => ({
            memName: m.memName,
            is_adult: m.is_adult,      // or Is_adult: m.is_adult
            labor_limit: m.labor_limit,
            familyID: currFamID,
        }));

        const { error: memberError } = await supabase.from("Members").insert(membersRows);
            ...
            <div className="login-container">
                <form className="login-form" onSubmit={handleSubmit}>
                    ...
                    <button
                        ...
                        onClick={() =>
                            setMembers([...members, { memName: "", is_adult: false, labor_limit: 1 }])
                        }
                    ...
                    <button className="signupbutton" type="submit" disabled={isSubmitting}>
                        <b>{isSubmitting ? "Creating..." : "Create account"}</b>
                    </button>
